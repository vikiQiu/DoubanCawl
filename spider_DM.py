...
def get_urls(info):
    ...
    regex_name=r'title="(.+?)"><img class="pil"'
...
def get_sep_info(urls):
    ...
    for url in urls:
        ...
        if len(stars)==0: 
            ...
            stars=stars[0]
        star.append(stars)
        comment_respon=str(len(re.findall(pat_respon,html)))
        respons.append(comment_respon)
...
"""得到所有页数的url"""
pattern="https://book.douban.com/subject/11542972/reviews?score=&start="
page_urls = get_page_urls(pattern,0,2)
print "page_urls OK"
...
print "sep_info OK"
for i in sep_info:
    print ("length is %d"%len(i))

f=open("../info/DM.txt",'w')
